...
class Attention(nn.Module):
    ...
    def compute_query_key_value_scores(self,
        ...
        '''
        # todo
        # query, key, value shape: (bs, n_local_heads, seqlen, head_dim)
        bs, n_heads, seqlen, head_dim = query.shape
        scores = torch.matmul(query, key.transpose(-2, -1))  # (bs, n_local_heads, seqlen, seqlen)
        scores = scores / (head_dim ** 0.5)  # Scale the attention scores
        if self.causal:
            # Apply causal mask
    ...
    def generate(self, idx, max_new_tokens, temperature=1.0, top_p=0.95):
        ...
        for _ in range(max_new_tokens):
            ...
            if temperature == 0.0:
                ...
                mask = probs_cumsum > top_p

                mask[..., 1:] = mask[..., :-1].clone()
                mask[..., 0] = False  # Never mask the top token
                ...
                idx_next = torch.multinomial(probs_sorted, num_samples=1)
