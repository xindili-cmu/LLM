...
def train_one_epoch(model, loader, optimizer, device):
    ...
    for x, y in loader:
        ...
        #     eq_pos = (x[i] == 12).nonzero()
        #     if len(eq_pos) > 0:
        #         eq_idx = eq_pos[0].item()
        #         # Mask positions after "="
        #         mask[i, eq_idx:] = True
...
def evaluate_loss(model, loader, device):
    ...
    for x, y in loader:
        ...
        # Create mask: only compute loss after "="
        mask = torch.zeros_like(y, dtype=torch.bool)
        for i in range(y.size(0)):
            # Find position of "=" token (id 12)
            eq_pos = (x[i] == 12).nonzero()
            if len(eq_pos) > 0:
                eq_idx = eq_pos[0].item()
                # Mask positions after "="
        ...
        # Compute cross entropy loss only on masked positions
        # Ignore positions where y == -1 (padding)
        valid_mask = mask_flat & (y_flat != -1)
        if valid_mask.sum() > 0:
